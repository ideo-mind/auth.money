...
import { useEVMWallet } from "@/src/mw/useEVMWallet";
...
const DEFAULT_AIRDROP_NATIVE_AMOUNT = parseUnits("10", 18); // 0.5 CTC
const DEFAULT_AIRDROP_TOKEN_AMOUNT = parseUnits("200", 6); // 100 USDC
...
router.post("/", async (c) => {
  try {
    ...
    const moneypotTokenAddress = (chain.custom as any).moneypot?.token?.address;
    if (!moneypotTokenAddress) {
      return c.json({ error: "MoneyPot token not deployed on chain" }, 400);
    }
    ...
    const AIRDROP_NATIVE_AMOUNT =
      (chain.nativeCurrency as any)?.airdrop ?? DEFAULT_AIRDROP_NATIVE_AMOUNT;
    const AIRDROP_TOKEN_AMOUNT =
      (chain.custom as any)?.moneypot?.token?.airdrop ?? DEFAULT_AIRDROP_TOKEN_AMOUNT;
    ...
    // 2. Transfer MoneyPot tokens
    if (!moneypotTokenAddress) {
      const errorResponse: AirdropErrorResponse = {
        error: `MoneyPot token not deployed on chain ${chainId}`,
      };
    ...
    const tokenTransferPromise = airdropWallet.writeContract({
      address: moneypotTokenAddress as `0x${string}`,
      abi: erc20Abi,
