#!/usr/bin/env python3
"""
Money Pot EVM End-to-End Application (Native ETH Payment)
Integrates with the verifier service for complete pot creation and hunting flow on EVM chains

This version uses native ETH payment for hunter attempts instead of token approvals.
The hunter pays the entry fee using native ETH, and the contract refunds any excess.
"""
...
class EVMVerifierServiceClient:
    ...
    async def register_verify(self, payload_json: str, signature: str) -> Dict[str, Any]:
        ...
        # payload_json is already a formatted JSON string that was signed
        # Don't re-serialize it!
...
class EVMMoneyPotApp:
    ...
    async def create_pot_flow(self, amount_wei: int = None, duration_seconds: int = None, fee_wei: int = None):
        ...
        async with self.verifier as verifier:
            ...
            # CRITICAL: Use compact JSON formatting to ensure 100% match with middleware
            # The middleware uses JSON.stringify() which by default adds spaces, so we match that
            # BUT - eth_account's encode_defunct expects the exact message that will be verified
            formatted_json = json.dumps(payload, separators=(',', ':'))
            print(f"Debug: JSON string being signed: {formatted_json}")

            # Create signature for the formatted JSON string
            ...
            print(f"✅ Sending formatted JSON: {formatted_json}")

            # CRITICAL: Send the formatted JSON string (not the object) to match what was signed
            register_result = await verifier.register_verify(formatted_json, signature_hex)
    ...
    async def _request_attempt(self, pot_id: str) -> int:
        """Request an attempt on the blockchain using native ETH payment"""
        # Get pot info to determine fee
        pot_info = get_pot_info(self.contract, int(pot_id))
        fee = pot_info.get('fee', 0)

        print(f"\n💰 Attempting with native ETH payment")
        print(f"   Entry Fee: {self.format_token_amount(fee)} tokens")
        
        # Check hunter's ETH balance
        hunter_balance = self.w3.eth.get_balance(self.hunter_account.address)
        hunter_balance_eth = self.w3.from_wei(hunter_balance, 'ether')
        print(f"   Hunter ETH Balance: {hunter_balance_eth} ETH")
        
        # Reserve some ETH for gas, send the rest
        gas_price = self.w3.eth.gas_price
        estimated_gas = 500000
        gas_reserve = gas_price * estimated_gas * 2  # Double for safety
        
        # Send almost the entire balance (contract will refund excess)
        eth_to_send = hunter_balance - gas_reserve if hunter_balance > gas_reserve else 0
        
        if eth_to_send <= 0:
            raise RuntimeError(f"Insufficient ETH balance for transaction. Have {hunter_balance_eth} ETH")
        
        eth_to_send_amount = self.w3.from_wei(eth_to_send, 'ether')
        print(f"   Sending: {eth_to_send_amount} ETH (contract will refund excess)")
...
async def main():
    """Main entry point"""
    print("Money Pot EVM End-to-End Application (Native ETH Payment)")
    print("=" * 50)
    print("✨ This version uses native ETH payment for hunter attempts")
    print("   No token approvals required for hunters")
    print("=" * 50)
